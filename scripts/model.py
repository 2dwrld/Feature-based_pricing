...
# Кастомный класс для StackingRegressor с возможностью установки параметров
...
# Функция для прогнозирования диапазона цен
def predict_price_range(model, input_data, X, y, input_categoricals):
    # Прогнозы оригинальной модели
    ...
    # Отладочные выводы для оригинальных прогнозов
    print(f"Model prediction: {prediction}")
    print(f"Predicted price range: {price_prediction_low} - {price_prediction_high}")

    # Получение рыночных цен с сайта auto.ru
    ...
    print(f"Market price standard otklonenie: {std_dev_market}")
